...
export default function Dropdown({index,datafaq}) {
  ...
  return (
    <div  key={index} className='dropdown-item'>
      <div onClick={()=>handleToggle(index)} className='header-dropdown-item'>
          <h3 className='h3-dropdown'>{datafaq.title}</h3>
          <img   src='./fetch.svg' className='img-arrow' style={{ transform:`rotate(${arrowOpen ? 180 : 0}deg)`, transition:'all 0.5s ease'}}  />
      </div>
